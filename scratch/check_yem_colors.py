...
for pid_str, tag in d['ownership'].items():
    if tag == 'YEM' and pid_str in meta['centers'] and 3000 < meta['centers'][pid_str]['x'] < 3500 and 1000 < meta['centers'][pid_str]['y'] < 1400:
        pid = int(pid_str)
        mask = (prov_ids == pid)
        if mask.any():
...
print("Colors of YEM provinces in India box:")
